...
| 7 | AI Agent tools | Multi-select | Claude Code (future: Codex, Gemini CLI, Copilot, Cline, Cursor, Amazon Q) |

Language は FW 選択で自動解決された言語を除外して表示する。全言語が自動解決済みならスキップする。

## Presets

20 presets, mapped 1:1 to wizard selections.
...
| 6 | Agent | 複数選択可 | `claude-code`, `codex`, `gemini`, `amazon-q`, `copilot` |
...
Application order: `base → typescript → python → react → nextjs → fastapi → express → aws → azure → gcp → cdk → cloudformation → terraform → bicep → claude-code → codex → gemini → amazon-q → copilot`
...
| terraform | via mise |
| CD workflow | `.github/workflows/cd-terraform.yaml` |
...
| `lefthook.yaml` | base, typescript, python, fastapi, express, cdk |
