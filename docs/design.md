...
8 questions. App-first flow: application frameworks first, then infrastructure, then testing, then agents.
...
| 7 | Testing tools | Multi-select | Playwright |
| 8 | AI Agent tools | Multi-select | Claude Code / Codex CLI / Gemini CLI / Amazon Q Developer / GitHub Copilot / Cline / Cursor |

Language は FW 選択で自動解決された言語を除外して表示する。全言語が自動解決済みならスキップする。

## Presets

28 presets, mapped 1:1 to wizard selections.
...
| `playwright` | Testing: Playwright | `typescript` (forced) |
...
| 6 | Testing | 複数選択可 | `playwright` |
| 7 | Agent | 複数選択可 | `claude-code`, `codex`, `gemini`, `amazon-q`, `copilot`, `cline`, `cursor` |
...
Application order: `base → typescript → python → react → nextjs → vue → nuxt → sveltekit → astro → hono → fastapi → express → batch → aws → azure → gcp → cdk → cloudformation → terraform → bicep → playwright → claude-code → codex → gemini → amazon-q → copilot → cline → cursor`
...
**Hono** (forces TypeScript) — adds:

| Element | Files |
|---------|-------|
| Hono app | `api/src/index.ts`, `api/src/app.ts` |
| Tests (app.request + vitest) | `api/tests/app.test.ts` |
| tsconfig (API) | `api/tsconfig.json` |
| Package (API) | `api/package.json` |
| Agent rules | `.claude/rules/hono.md` |
...
**Playwright** (forces TypeScript) — adds:

| Element | Files |
|---------|-------|
| Playwright config | `e2e/playwright.config.ts` |
| Example spec | `e2e/example.spec.ts` |
| Package (E2E) | `e2e/package.json` |
| MCP: Playwright | distributed to agent configs |
...
| Backend のみ (Hono/Express) | `api/` | pnpm-workspace |
| Backend のみ (FastAPI) | `api/` | なし（Python/uv） |
| Frontend + FastAPI | `web/` + `api/` | pnpm-workspace（web のみ） |
| Frontend + Hono/Express | `web/` + `api/` | pnpm-workspace（web + api） |

**Workspace ルール:**

- `web/`（全 Frontend）→ pnpm workspace に含める
- `api/`（Hono / Express）→ pnpm workspace に含める
- `api/`（FastAPI）→ uv で管理、pnpm workspace に含めない
- `worker/`（Batch）→ pnpm workspace に含める
- `e2e/`（Playwright）→ pnpm workspace に含める
...
### Built-in vs external presets

The 28 presets above are **built-in** — bundled with the CLI. The CLI also
loads **external presets** at runtime:

- **CLI flag**: `--preset <module>` (repeatable, accepts npm package name or
  relative path)
- **Project config**: `agentic-app.config.json` with a `presets: string[]` field

External presets satisfy the same `Preset` interface and use the same merge
semantics. They are applied **after** all built-in presets in the order
supplied. External preset names may not collide with built-ins. See
[`docs/preset-authoring.md` § External Preset Packages](preset-authoring.md#external-preset-packages)
...
|-------------|-------------|
| `package.json` | base, typescript, python, react, nextjs, vue, nuxt, sveltekit, astro, hono, fastapi, express, batch, cdk, bicep, playwright |
| `.mise.toml` | base, typescript, python, aws, azure, gcp, cdk, cloudformation, terraform |
| `lefthook.yaml` | base, typescript, python, hono, fastapi, express, batch, cdk |
| `pnpm-workspace.yaml` | react, nextjs, vue, nuxt, sveltekit, astro, hono, express, batch, playwright (auto-generated when workspace packages exist) |
| `.github/workflows/ci.yaml` | base, typescript, python, hono, fastapi, express, batch, cdk, cloudformation, terraform, bicep, playwright |
| `.github/workflows/cd-*.yaml` | cdk, cloudformation, terraform, bicep (each preset owns its own CD workflow) |
| `.mcp.json` | aws, azure, gcp, playwright (MCP servers distributed to agent config files) |
| `.vscode/settings.json` | typescript, python, nextjs, nuxt, sveltekit, cdk |
| `.vscode/extensions.json` | typescript, python, cdk, bicep, terraform |
| `.devcontainer/devcontainer.json` | typescript, python, react, nextjs, vue, nuxt, sveltekit, astro, hono, fastapi, express, aws, azure, gcp, cdk, terraform, bicep, claude-code, codex, gemini |
...
| MCP: GitHub / PostgreSQL | Require project-specific credentials; documented in `.mcp.json.example` |
...
- Hono + React（web/ + api/、pnpm workspace）
- Hono + CDK（api/ + infra/ の共存）
- SvelteKit + Hono（web/ + api/、pnpm workspace）
- Astro + FastAPI（web/ + api/、TypeScript + Python）
- React + Playwright（web/ + e2e/ の共存）
