...
21 presets, mapped 1:1 to wizard selections.
...
Application order: `base → typescript → python → react → nextjs → fastapi → express → aws → azure → gcp → cdk → cloudformation → terraform → bicep → claude-code → codex → gemini → amazon-q → copilot → cline → cursor`
...
| `.devcontainer/devcontainer.json` | typescript, python, fastapi, aws, azure, gcp, cdk, bicep, codex, gemini |
