...
| ツール | 役割 |
|-------|------|
| `release-please.yaml` | main への push で version bump PR を自動作成（CHANGELOG 生成含む） |
| `release.yaml` | GitHub Release 作成時に lint → test → build → npm publish を実行 |
...
- `release.yaml` が実行されるが、Trusted Publishing 未設定のため npm publish は失敗する（想定通り）
