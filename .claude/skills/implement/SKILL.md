...
- AskUserQuestion で「何を実装しますか？（Issue 番号 or 説明）」と確認する
...
2. 要件から `<type>/<slug>` 形式のブランチ名を決定する
   - 例: `feat/add-auth`, `fix/cdk-synth-error`
...
### 変更内容
1. `src/index.ts` — エントリポイントにルーティング追加
2. `infra/lib/app-stack.ts` — Lambda 関数リソースを定義
3. `tests/test_handler.py` — ハンドラーのテストを追加

### 影響範囲
- 既存の AppStack にリソース追加（既存機能に影響なし）

承認しますか？
```

1. AskUserQuestion で確認: 「この計画で実装」「計画を修正」「キャンセル」
...
2. **テスト作成:** 必要に応じてテストを作成する
3. **進捗報告:** 各ファイルの変更完了時に簡潔に報告する

実装中に計画の変更が必要になった場合は、ユーザーに相談する。

### Step 5: 動作確認

実装完了後、ユーザーに報告する**前に**必ず動作確認を行う:

1. `pnpm run build` — ビルドが通ることを確認
2. `pnpm test` — 全テストが通ることを確認
3. `pnpm run typecheck` — 型チェックが通ることを確認
