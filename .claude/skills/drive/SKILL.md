...
**自動実行:** implement Step 3 の計画承認（AskUserQuestion）は**スキップ**する。`/drive` を実行した時点でユーザーは自律実行を委任しているため、計画を自ら承認して実装を進める。
...
1. `.claude/skills/ship/SKILL.md` の Step 1〜4 を実行する（lint → test → commit → push。動作確認は ship 内の lint・test で担保される）
2. 3a に戻る
