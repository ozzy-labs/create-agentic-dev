...
`.claude/skills/implement/SKILL.md` を Read し、Step 1〜5 を実行する（Step 6 は無視する）。

**ユーザー介入ポイント:**

- 実装計画の承認（implement Step 3）— 承認なしにコード変更を開始しない

**中断条件:**

- ユーザーが計画をキャンセルした場合 → 全体を中断
- 動作確認（build/test/typecheck）が繰り返し失敗する場合 → エラーを報告して中断

implement が完了したら、ユーザー確認なしに Phase 2 に進む。

### Phase 2: ship

`.claude/skills/ship/SKILL.md` を Read し、Step 1〜5 を実行する（Step 6 は無視する）。

- lint → test → commit → PR 作成を順に実行する
- PR 番号を記録する（Phase 3 で使用）

**中断条件:**

- lint またはテストが失敗し、自動修正できない場合 → エラーを報告して中断
...
`.claude/skills/review/SKILL.md` を Read し、Step 1〜4 を実行する（Step 5 は無視する）。
...
1. 動作確認を実行する（`pnpm run build && pnpm test && pnpm run typecheck`）
2. `.claude/skills/ship/SKILL.md` の Step 1〜4 を実行する（lint → test → commit → push）
3. 3a に戻る
...
```text
/drive 完了:
  Issue:    #105 feat: add Cline preset
  ブランチ: feat/add-cline-preset
  PR:       https://github.com/owner/repo/pull/110
  レビュー: 2 回実施（Critical: 0, Warning: 0, Info: 1）
  状態:     merge-ready
```
...
```text
  Info 指摘（未修正）:
    - [Info] docs/design.md:24 — 表現の改善提案
```
...
- **implement の計画承認は省略しない**（唯一のユーザー介入ポイント）
